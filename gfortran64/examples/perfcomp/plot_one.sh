...
set names = "acml netlib openblas atlas"

foreach name ( ${names} )

set n = -${name}
test -r ${plot1_d}${n} || continue

touch csplit-0 0-thread
rm -f csplit-* *-thread
#csplit -k -s -f csplit- ${plot1_d} /threads/ '{20}' >& /dev/null
csplit -k -s -f csplit${n}- ${plot1_d}${n} /threads/ '{20}' >& /dev/null
#rm -f csplit-00 csplit2-00
rm -f csplit${n}-00
foreach file (`ls csplit${n}-*`)
  mv ${file} `head -1 $file | sed -e "s/.* \([0-9]*\) threads/\1-thread${n}/"`
end
...
set has = 0
foreach file (`ls *-thread${n}`)
  set has = 1
  set qp1 = \'${file}\'
  if ("${plotline}" != "replot" && "${plotline}" != "plot") then
    set plotline = "${plotline},"
  endif
  set plotline = "${plotline} ${qp1}, ${qp1} with lines"
end
if ( ${has} == 1 ) then
  echo "${plotline} title '${name}'" >> ${temp_file}
  set plotline = "replot"
endif
